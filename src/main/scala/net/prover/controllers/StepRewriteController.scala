...
import net.prover.proving.rewrite.{RewritePremise, RewriteStep}
...
class StepRewriteController @Autowired() (implicit val bookService: BookService) extends ChainingStepEditing {
  ...
  private def getRewriteStepForInference[TExpression <: Expression & TypedExpression[TExpression] : RewriteMethods](
    inferenceId: String,
    baseTerm: Term,
    ...
    direction: Direction,
    equality: Equality)(
    using stepProvingContext: StepProvingContext
  ): Try[RewriteStep[TExpression]] = {
    ...
    for {
      ...
      (appliedExtraction, _) <- ExtractionApplier.applyInferenceExtractionWithoutPremises(inferenceExtraction, finalSubstitutions).orBadRequest("Could not apply extraction")
    } yield RewriteStep(removedSource, rewrittenTerm, RewritePremise.ByInference(knownPremises, appliedExtraction), removedUnwrappers, removedWrapperExpression)
  }
  ...
  def getRewriteStepForPremise[TExpression <: Expression with TypedExpression[TExpression] : RewriteMethods](
    ...
    } yield RewriteStep(premiseLhs, premiseRhs, RewritePremise.Known(knownStatement), removedUnwrappers, removedWrapperExpression)
  }
  ...
  def rewrite[TExpression <: Expression with TypedExpression[TExpression] : RewriteMethods, TStep](
    ...
    equality: Equality,
    direction: Direction)(
    applyRewrite: (Term, Term, Seq[Unwrapper], Wrapper[Term, TExpression], RewritePremise) => Try[(TStep, Option[Inference.Summary], Wrapper[Term, TExpression])])(
        ...
        for {
          ...
          (newTarget, steps, inferences) <- direction.reverseIfNecessary(rewrites).foldLeft(Try((currentExpression, Seq.empty[TStep], Seq.empty[Option[Inference.Summary]]))) { case (trySoFar, rewrite) =>
            for {
              ...
              RewriteStep(term, rewrittenTerm, rewritePremise, unwrappers, wrapperExpression) <- ((rewrite.inferenceId.map(getRewriteStepForInference(_, baseTerm, rewrite, baseUnwrappers, function, direction, equality)(using summon, stepWithContext.stepProvingContext.withStepContext(replacementStepContext))) orElse
              ...
              (step, resultInferenceOption, updatedWrapper) <- applyRewrite(source, result, unwrappers, wrapper, rewritePremise)
  ...
  def substituteForRewrite(
    ...
    val inference = premise.explicitInference.getOrElse(equality.substitution.inference.summary)
    ...
    val rewriteStep = Step.RewriteStep.ifNecessary(premise, substitutionStep)
