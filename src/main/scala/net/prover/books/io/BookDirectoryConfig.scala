...
object BookDirectoryConfig {
  ...
  def getProofPath(bookTitle: String, chapterTitle: String, chapterIndex: Int, theorem: Theorem, theoremIndex: Int, proofIndex: Int): Path = {
    getChapterDirectoryPath(bookTitle, chapterTitle, chapterIndex).resolve("%02d".format(theoremIndex + 1) + "." + theorem.title.formatAsKey + "." + "%02d".format(proofIndex + 1) + ".proof")
  }
}
