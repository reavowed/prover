  ...
  override def updateAutogenerated(step: Step.Autogenerated, stepWithContext: StepWithContext): Step = {
    step match {
      ...
      case step: Step.ExistingStatementExtractionStep =>
        step.substeps.head match {
          case assertionStep: AssertionStep if stepWithContext.stepProvingContext.provingContext.availableEntries.statementDefinitions.exists(_.deconstructionInference.contains(assertionStep.inference)) =>
            apply(stepWithContext.withStep(step), assertionStep.inference) getOrElse super.updateAutogenerated(step, stepWithContext)
          case _ =>
