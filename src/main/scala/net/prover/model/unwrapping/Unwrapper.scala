...
sealed trait Unwrapper {
  ...
  def enhanceContext(implicit stepContext: StepContext): StepContext
  ...
  def rewrapWithDistribution(steps: Seq[Step], joiner: BinaryJoiner[Statement], source: Statement, result: Statement)(implicit stepContext: StepContext): Try[Seq[Step]]
...
case class GeneralizationUnwrapper(variableName: String, generalizationDefinition: GeneralizationDefinition, specificationInference: Inference) extends Unwrapper {
  ...
  def rewrapWithDistribution(steps: Seq[Step], joiner: BinaryJoiner[Statement], source: Statement, result: Statement)(implicit stepContext: StepContext): Try[Seq[Step]] = {
    for {
      distributionInference <- stepContext.provingContext.generalizationDistributions.get(joiner).orBadRequest(s"Could not find generalization distribution inference for ${joiner.symbol}")
...
case class DeductionUnwrapper(antecedent: Statement, deductionDefinition: DeductionDefinition, deductionEliminationInference: Inference) extends Unwrapper {
  ...
  def rewrapWithDistribution(steps: Seq[Step], joiner: BinaryJoiner[Statement], source: Statement, result: Statement)(implicit stepContext: StepContext): Try[Seq[Step]] = {
    for {
      distributionInference <- stepContext.provingContext.deductionDistributions.get(joiner).orBadRequest(s"Could not find deduction distribution inference for ${joiner.symbol}")
...
object Unwrapper {
  ...
  implicit class Unwrappers(unwrappers: Seq[Unwrapper]) {
    ...
    def rewrapWithDistribution(joiner: BinaryJoiner[Statement], source: Term, result: Term, steps: Seq[Step], wrapper: Wrapper[Term, Statement], inference: Option[Inference])(implicit stepContext: StepContext): Try[(Seq[Step], Wrapper[Term, Statement])] = {
        ...
        currentUnwrappers match {
          case unwrapper +: tailUnwrappers =>
            ...
            for {
              ...
              rewrappedSteps <- unwrapper.rewrapWithDistribution(innerSteps, joiner, innerSource, innerResult)(currentStepContext)
