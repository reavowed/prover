...
case class EqualityRewriter(equality: Equality)(implicit stepProvingContext: StepProvingContext)
{
  ...
  private case class StepWithInference(step: Step, inference: Option[Inference.Summary])
...
object EqualityRewriter {
  ...
  object RewriteMethods {
    def apply[T <: Expression](implicit replacementMethods: RewriteMethods[T]): RewriteMethods[T] = replacementMethods
    ...
    object StatementRewriteMethods extends RewriteMethods[Statement] {
      def getRewritePossibilitiesFromExpression(statement: Statement, path: Seq[Int], unwrappers: Seq[Unwrapper], wrapper: Statement => Statement)(implicit stepContext: StepContext): Seq[RewritePossibility[Statement]] = {
