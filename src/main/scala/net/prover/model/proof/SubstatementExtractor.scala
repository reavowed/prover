...
class SubstatementExtractor(implicit stepProvingContext: StepProvingContext) {
  ...
  private def simplify[T](extractionCandidate: Statement, termsSoFar: Int, recurse: (Statement, Int) => Result[T]): Result[T] = {
    premiseSimplificationInferences.iterator.flatMap { case (inference, singlePremise) =>
      for {
        ...
        (terms, innerSteps, t) <- recurse(extractedConclusion, termsSoFar)
        substitutedPremise <- extractionCandidate.specify(terms).toSeq
  ...
  private def deconstructDefinition[T](extractionCandidate: Statement, termsSoFar: Int, recurse: (Statement, Int) => Result[T]): Result[T] = {
    for {
      ...
      (terms, innerSteps, t) <- recurse(deconstructedStatement, termsSoFar)
  ...
  private def extractWithRewrite[T](extractionCandidate: Statement, termsSoFar: Int, extractWithoutRewrite: (Statement, Int) => Result[T]): Result[T] = {
    extractWithoutRewrite(extractionCandidate, termsSoFar) ++ rewriteInferences.iterator.flatMap { case (inference, singlePremise) =>
      for {
        extractionSubstitutions <- singlePremise.calculateSubstitutions(extractionCandidate).flatMap(_.confirmTotality).toSeq
