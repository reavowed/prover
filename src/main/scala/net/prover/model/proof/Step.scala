...
object Step {
  ...
  case class InferenceWithPremiseDerivations(premiseDerivationSteps: Seq[Step], assertion: Step) extends Step.Autogenerated {
    val `type` = "inferenceWithPremiseDerivations"
    override def substeps: Seq[Step] = premiseDerivationSteps :+ assertion
    override def replaceSubsteps(newSubsteps: Seq[Step], stepContext: StepContext): Step = InferenceWithPremiseDerivations(newSubsteps)
  }
  object InferenceWithPremiseDerivations {
    def apply(steps: Seq[Step]): InferenceWithPremiseDerivations = {
      InferenceWithPremiseDerivations(steps.init, steps.last)
    }
