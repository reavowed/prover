...
object Step {
  ...
  sealed trait InferenceApplicationWithoutPremises extends InferenceApplication with PremiseDerivation {
    ...
    def addPremiseDerivations(premiseDerivations: Seq[InferenceApplicationWithoutPremises]): InferenceApplication = {
      if (premiseDerivations.isEmpty) {
        ...
        InferenceWithPremiseDerivations(Nil, premiseDerivations, this)
      }
    }
  }
  ...
  case class ExistingStatementExtraction(substeps: Seq[Step.InferenceApplicationWithoutPremises]) extends Step.Autogenerated {
      ...
      with Step.Wrapped
  {
    ...
    override def inference: Inference.Summary = assertion.inference
    override def innerSteps: Seq[Step] = premiseDerivationSteps :+ assertion
    override def replaceSubstepsInternal(newSubsteps: Seq[Step], stepContext: StepContext): Step = InferenceWithPremiseDerivations(newSubsteps)(stepContext.provingContext)
  }
  object InferenceWithPremiseDerivations {
    def apply(steps: Seq[Step])(implicit provingContext: ProvingContext): InferenceWithPremiseDerivations = {
      Wrapped.extractWrappers(steps) { (unwrappers, steps) =>
