...
object Step {
  ...
  case class ExistingStatementExtraction(substeps: Seq[Step]) extends Step.Autogenerated {
    ...
    override def replaceSubsteps(newSubsteps: Seq[Step], stepContext: StepContext): Step = ExistingStatementExtraction(substeps)
  }
  ...
  case class PremiseDerivation(premiseDerivationSteps: Seq[Step], assertion: Step) extends Step.Autogenerated {
    val `type` = "premiseDerivation"
    override def substeps: Seq[Step] = premiseDerivationSteps :+ assertion
    override def replaceSubsteps(newSubsteps: Seq[Step], stepContext: StepContext): Step = PremiseDerivation(newSubsteps)
  }
  object PremiseDerivation {
    def apply(steps: Seq[Step]): PremiseDerivation = {
      PremiseDerivation(steps.init, steps.last)
    }
    def parser(implicit availableEntries: AvailableEntries, stepContext: StepContext): Parser[PremiseDerivation] = {
      for {
        substeps <- listParser.inBraces
      } yield PremiseDerivation(substeps)
    }
  }
  ...
  def parser(implicit availableEntries: AvailableEntries, stepContext: StepContext): Parser[Option[Step]] = {
    Parser.selectOptionalWordParser {
      ...
      case "premiseDerivation" => PremiseDerivation.parser
    }
  }
