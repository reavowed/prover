...
import net.prover.model.proof.{DerivationStep, Step, SubstitutionContext}
import net.prover.model.{Inference, Substitutions}

sealed trait DerivedPremise {
  def getRootPremises: Seq[Statement]
  def getSubstitutedPremises(substitutions: Substitutions)(implicit substitutionContext: SubstitutionContext): Option[(Seq[Statement], Seq[DerivationStep])]
}
