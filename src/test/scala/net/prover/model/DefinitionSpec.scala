...
class DefinitionSpec extends ProverSpec {
  "definition parser" should {
    "parse a definition" in {
      Definition.parse("and not implies 1 not 2", defaultContext) mustEqual
        Definition(Conjunction, Negation(Implication(1, Negation(2))))
    }
  }
  ...
  "definition" should {
    "apply to a theorem with the defined connective" in {
      ...
      val updatedTheorem = definition.readAndUpdateTheoremBuilder(theorem, "1", defaultContext)
