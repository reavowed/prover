...
class InferenceSpec extends Specification {
  "inference id" should {
    "be independent of inference name" in {
      createInference("XXX", Seq(φ), ψ).id must beEqualTo(createInference("YYY", Seq(φ), ψ).id)
    }
    "be independent of bound variable names" in {
      createInference("XXX", Nil, ForAll("x")(φ($))).id must beEqualTo(createInference("YYY", Nil, ForAll("x")(φ($))).id)
    }
