...
class TheoremBuilderSpec extends ProverSpec {
  "theorem builder" should {
    "handle simple direct rule" in {
      ...
      val updatedTheoremBuilder = rule.readAndUpdateTheoremBuilder(theoremBuilder, "h1", Book(""))
    ...
    "handle direct rule referencing fantasy hypothesis" in {
      ...
      val updatedTheoremBuilder = rule.readAndUpdateTheoremBuilder(theoremBuilder, "f.h", Book(""))
    ...
    "fail an assumption discharging rule when theorem builder has no fantasy" in {
      ...
      rule.readAndUpdateTheoremBuilder(TheoremBuilder(), "", Book("")) must throwAn[Exception]
    }

    "handle simple assumption discharging rule" in {
      val rule = FantasyRule("???", Atom(1), Nil, Atom(1))
      val theoremBuilder = TheoremBuilder().addFantasy(Atom(1))
      val updatedTheoremBuilder = rule.readAndUpdateTheoremBuilder(theoremBuilder, "", Book(""))
    ...
    "handle assumption discharging rule with premise" in {
      ...
      val updatedTheoremBuilder = rule.readAndUpdateTheoremBuilder(theoremBuilder, "f.1", Book(""))
    ...
    "handle assumption discharging rule applied to theorem" in {
      ...
      val theoremBuilder = rule.readAndUpdateTheoremBuilder(TheoremBuilder(), "and-sym 2 1", Book("", theorems = Seq(theorem)))
